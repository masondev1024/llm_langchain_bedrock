...
class MCPClient:
    ...
    def __init__(self, server_script: str = 'server.py'):
        ...
        self.tools = [] # MCP 서버에게 툴 목록가져와서 저장
        pass
    async def run(self):
            ...
            async with stdio_client(server_params) as (read,write):
                async with ClientSession(read,write) as session:
                    ...
                    for i, tool in enumerate(self.tools,1):
                        ...
                        if hasattr(tool,"inputSchema" ) and tool.inputSchema: # 해당 멤버변수가 있는가? -> 값은 있는가
                            props=tool.inputSchema.get('properties',{}) # 키->값 추출(인덱싱), 비워있는 경우 대비 초기값 설정
                            if props:
